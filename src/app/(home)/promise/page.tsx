    ...
    <main>
      ...
      <div className="mt-3 flex px-6 gap-2">
        <div className="relative flex-1 h-11 bg-[#F5F5F5] rounded-full flex">
          ...
          <input type="text" className='bg-transparent focus:outline-none w-full h-full focus:outline-red-200 rounded-full pl-10 pr-5' placeholder='Cari Dokter' />
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 mt-5">
            {
              dokter.map((person, index) => (
                <CardDokter image={person.photo} key={index}>
                    <p className="text-[10px] mt-3 text-center">{ person.name }</p>
                    <p className="text-[10px] text-center text-red-600">{ person.specialist }</p>
                </CardDokter>
              ))
            }
        </div>
